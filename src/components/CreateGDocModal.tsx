...
import { createGDoc } from "@/app/actions/document";
import { createTipoDocumento } from "@/app/actions/tipoDocumento";

export default function CreateGDocModal({ processoId, tipos }: { processoId: string, tipos: { id: string, nome: string }[] }) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await createGDoc(formData);
      setIsOpen(false);
      setIsNewTipo(false);
      setNovoTipoNome("");
    } catch (err: any) {
      setError(err.message || "Erro desconhecido ao criar documento.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <>
      <button 
        ...
        className="flex items-center gap-2 px-3 py-2 bg-blue-50 hover:bg-blue-100 text-blue-700 text-sm font-medium rounded transition-colors"
      >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-semibold text-slate-800">Criar Google Doc</h2>
